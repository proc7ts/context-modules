...
export class CxModule$Impl {
  ...
  private readonly _setup: (this: void, setup: CxModule.Setup) => void | PromiseLike<unknown>;

  constructor(
    readonly module: CxModule,
    readonly name: string,
    readonly options: CxModule.Options,
  ) {
...
export function CxModule$replace(
  replaced: CxModule,
  replacement: CxModule,
): CxAsset<CxModule.Handle, CxModule> {
  return {
    entry: replaced,
...
function CxModule$loadDeps(
  ...
  return Promise.race([
    whenDone,
    Promise.all(
      deps.map(({ dep, use }) => whenLoaded(use).then(noop, error => [dep, error] as const)),
    ).then((results): true | CxDependencyError => {
